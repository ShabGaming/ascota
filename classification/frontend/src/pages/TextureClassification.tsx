...
import {
  runTextureClustering,
  getTextureResults,
  getSession,
  reclusterTexture,
  updateTextureResult,
  setTextureClusterName,
  undoTextureEdit,
  exportSession,
  getImageUrl,
  TextureRunResponse,
...
function textureRunToResultsView(data: TextureRunResponse): TextureResultsResponse {
  const results: Record<string, { cluster_id: number }> = {}
  const rows = data.results
  if (Array.isArray(rows)) {
    for (const row of rows) {
      if (row && typeof row.item_id === 'string') {
        results[row.item_id] = { cluster_id: Number(row.cluster_id) }
      }
    }
  }
  return {
    ...
    noise_item_ids: data.noise_item_ids ?? [],
  }
}
...
const DEFAULT_CLUSTER_SELECTION_METHOD = 'leaf'
...
function TextureClassification({ sessionId, onReset }: TextureClassificationProps) {
  ...
  const applyResults = useCallback((data: TextureResultsResponse) => {
  ...
  const fetchResults = useCallback(async () => {
    try {
      const data = await getTextureResults(sessionId)
      applyResults(data)
    } catch {
    ...
    getSession(sessionId)
      .then((session) => {
        if (session.has_results) {
          setHasRun(true)
          return getTextureResults(sessionId).then((data) => {
  ...
  const handleRun = async () => {
    setRunLoading(true)
    try {
      const data = await runTextureClustering(sessionId, { resolution })
      setHasRun(true)
      applyResults(textureRunToResultsView(data))
      toast({ title: 'Texture clustering complete', status: 'success', duration: 2000 })
    } catch (err: unknown) {
  ...
  const handleRecluster = async () => {
    ...
    try {
      ...
      const data = await reclusterTexture(sessionId, options)
      applyResults(data)
  ...
  const handleUndo = async () => {
    try {
      const data = await undoTextureEdit(sessionId)
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast({
        title: 'Exported',
        description: `Saved to ${data.saved_finds} find(s) .ascota/classification.json under key "texture".`,
        status: 'success',
        duration: 4000,
      })
    } catch {
  ...
  const handleMove = async (itemId: string, clusterId: number) => {
    ...
    try {
      await updateTextureResult(sessionId, itemId, clusterId)
      await fetchResults()
  ...
  const handleRenameCluster = async (clusterId: number, name: string) => {
    ...
    if (!trimmed) return
    try {
      await setTextureClusterName(sessionId, clusterId, trimmed)
    ...
    return (
      <Container maxW="container.lg" py={10}>
        <VStack spacing={6}>
          ...
          <Text color="gray.600">Texture results were saved to each find&apos;s .ascota/classification.json.</Text>
    ...
    <Container maxW={!hasRun ? 'container.lg' : 'container.xl'} py={!hasRun ? 10 : 6}>
      <VStack spacing={6} align="stretch">
        <HStack justify="space-between" flexWrap="wrap">
          <Heading size="lg">Texture clustering</Heading>
        ...
        {!hasRun ? (
            ...
            <Box>
              <VStack align="stretch" spacing={4} maxW="md" mx="auto">
                <Text fontSize="sm" color="gray.600">
                  Grayscale LBP + GLCM features (opaque region only), then PCA + HDBSCAN. Same workflow as color
                  clustering.
                </Text>
                <FormControl>
                ...
                <Button
                  ...
                  loadingText="Extracting texture features and clustering..."
                >
                  Run texture clustering
                </Button>
              </VStack>
            </Box>
            {runLoading && (
              ...
              >
                <VStack spacing={4} bg="white" p={8} borderRadius="lg">
                  <Spinner size="xl" />
                  <Text fontWeight="medium">Extracting texture features and clustering...</Text>
                </VStack>
              </Flex>
            )}
          </VStack>
          ...
          <>
            <Wrap spacing={4} align="center">
              <WrapItem>
                <Tooltip label="Minimum cluster size; smaller groups become noise.">
                  <FormControl w="100px">
              ...
              <WrapItem>
                <Tooltip label="1 recommended to avoid most points labeled noise.">
                  <FormControl w="90px">
                    <FormLabel fontSize="xs" mb={1}>
                      min_samples
                    </FormLabel>
              ...
              <WrapItem>
                <Tooltip label="leaf = finer clusters; eom = often fewer noise points.">
                  <FormControl w="100px">
                    <FormLabel fontSize="xs" mb={1}>
                      method
                    </FormLabel>
...
export default TextureClassification
